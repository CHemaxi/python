...
* Save the file as `test-data.xml` in `c:/learnpython` folder
MARKDOWN """
# MARKDOWN```xml
# <learnpython>
#     <training>
...
# xmldoc = minidom.parse('C:/learnpython/test-data.xml')
xmldoc = minidom.parse('C:/learnpython/test-data.xml')
...
################################################################################
# Extract the LEARNPYTHON element and its children from the xmldoc
learnpython_elements = xmldoc.getElementsByTagName("learnpython")


################################################################################
# Loop through a Tag Element and read underlying Elements / Attributes
################################################################################
# Since we know the learnpython element has children we use the loop
for learnpython_element in learnpython_elements:

    # For every learnpython element, extract the next clind node, i.e, training
    training_elements = learnpython_element.getElementsByTagName("training")
...
# SAXprocessXML('c:\\learnpython\\test.xml')
SAXprocessXML('C:/learnpython/test-data.xml')
...
# Create Root Element
root_element = ET.Element("learnpython")
...
myfile = open("C:/learnpython/test-data-OUT.xml", "w")
myfile.write(mydata)
# MARKDOWN```
